...
Therefore, all observations can be compactly represented by $N\times M$ matrix $\mathbf{y}$,
such that each row $i$ is the observed star $\mathbf{y}_{i}$. The central pixel of the
postage stamp is chosen such that the estimated centroid of the star $(x_i , y_i)$ is 
in the central pixel. Let us denote the vector connecting the the center of the postage-stamp
of star $i$ to its centroid by $(\delta x_{i}, \delta y_{i})$, and the flux of star by $f_{i}$.
An uncertainty $\sigma_{ij}$ is associated with $j$-th pixel of the $i$-th star. 
If the only contribution to per-pixel uncertainty is uncorrelated Gaussian noise,
we will have $\sigma_{ij} = \sigma_{i}\delta_{ij}$.
However, one can use a more general form of uncertainty to account for the
effect of outlier pixels such as those hit by Cosmic rays, or overlapping stars.
